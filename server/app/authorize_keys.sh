...
log() {
  # stderr so sshd logs can capture it
  echo "AUTH_KEYS: $*" >&2
}
...
log "invoked user=${user} key_type=${key_type:-<empty>} key_b64_len=${#key_b64}"

# Always accept the presented key for put/get (no signup required).
if [ "$user" = "put" ] || [ "$user" = "get" ]; then
  if [ -n "$key_type" ] && [ -n "$key_b64" ]; then
    log "accepting presented key for user=${user}"
    printf "%s %s\n" "$key_type" "$key_b64"
    exit 0
  fi
  log "no key presented for user=${user}"
fi

# No fallback to pre-provided keys.
log "no key emitted for user=${user}"
exit 0
